...
function HeroSlider() {
  ...
  const imgSrc = `/test-images/${SAMPLE6_FILENAME}`
  ...
  return (
    ...
    >
      {/* Right side: "reconstructed" — same image, shown clean */}
      <img src={imgSrc} alt="Reconstructed MRI"
        className="absolute inset-0 w-full h-full object-cover"
        style={{ filter: 'grayscale(100%)' }} />

      {/* Left side: "corrupted" — same image with degradation overlay */}
      <div className="absolute inset-0 overflow-hidden"
        style={{ clipPath: `inset(0 ${100 - sliderX}% 0 0)` }}>
        <img src={imgSrc} alt="Corrupted MRI"
          className="absolute inset-0 w-full h-full object-cover"
          style={{ filter: 'grayscale(100%) contrast(0.75) brightness(1.15) blur(0.6px)' }} />
